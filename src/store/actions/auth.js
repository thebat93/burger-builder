...
export const auth = (email, password, isSignup) => {
  return dispatch => {
    ...
    // URL для регистрации
    let url =
      'https://www.googleapis.com/identitytoolkit/v3/relyingparty/signupNewUser?key=AIzaSyCyqOtfIX5Um1BfCT_Y-ZgrTcUNLCxYDtQ';
    if (!isSignup) {
      // URL для входа
      url =
        'https://www.googleapis.com/identitytoolkit/v3/relyingparty/verifyPassword?key=AIzaSyCyqOtfIX5Um1BfCT_Y-ZgrTcUNLCxYDtQ';
    }
...
export const authCheckState = () => {
  return dispatch => {
    ...
    if (!token) {
      ...
      // иначе выполняем действие authSuccess
